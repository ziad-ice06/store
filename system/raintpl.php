...
class RainTPL{
	...
	// -------------------------
		...
		/**
		 * Cache . Is the directory where RainTPL will compile the template and save the cache
		 *
		 * @var string
		 */
	...
	function draw( $tpl_name, $return_string = false ){				
			...
			//////////////added by Rana for Globaly passing base url to every tpl file :D /////////////
		$this->var[ "base_url" ] = self::$base_url;
	...
	private function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			$cache_dir = self::$cache_dir;	// cache directory
	...
	private function compileTemplate( $template_code, $tpl_basedir ){
		...
		//tag list
		$tag_regexp = array( 	'gettext' 		=> '(\{gettext(?: name){0,1}="\${0,1}(?:.*?)"\})',//for gettext parsing
	...
	private function compileCode( $parsed_code ){
		...
		$parent_loop[ $level = 0 ] = $loop_name = $compiled_code = $open_if = $comment_is_open = $ignore_is_open = null;
	 	...
	 	while( $html = array_shift( $parsed_code ) ){
			...
			//gettext
			elseif( preg_match('/\{gettext(?: name){0,1}="\${0,1}(.*?)"\}/', $html, $code ) ){
				...
				//loop code
				//if(strcmp($var , "This is how the story goes.\n\n"))
				//{	$varr = "'".$var."'";}
				//$varr = trim($code[1]," ");
				$compiled_code .= _($var);
				//$compiled_code .= _("This is how the story goes.\n\n");
